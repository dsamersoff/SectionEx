...
// 		$article = & $this->articles[$i];
	...
	<?php
	    ...
	    <?php /*  article title data */ ?>
	        ...
	        <?php if (($this->params->get('se_show_create_date') == 2) || ($this->params->get('se_show_modify_date') == 2) || ($this->params->get('se_show_author') == 2) || ($this->params->get('se_show_voting') == 2) || ($this->params->get('se_show_hits') == 2) ): ?>
	        
				
				<?php if ($this->params->get('se_show_create_date') == 2) { ?>
					<div class="created"><?php echo JText::_('CREATED') .": ". SectionExHelper::formatDate( $article->created, $this->params ); ?></div>
				<?php } ?>
							
			    
				<?php if ($this->params->get('se_show_modify_date') == 2) { ?>
			        <div class="modified"><?php echo JText::_('UPDATED') .": ". SectionExHelper::formatDate( $article->modified, $this->params ); ?></div>
				<?php } ?>
				
				
				<?php if ($this->params->get('se_show_author') == 2) { ?>
			    	<div class="author"><?php echo JText::_('AUTHOR') .": ". ($article->created_by_alias ? $article->created_by_alias : $article->author); ?></div>
				<?php } ?>
					
							
				<?php if ($this->params->get('se_show_voting') == 2) { ?>
					<div class="votes">
						<?php echo JText::_('RATING') .": ". ($article->voting ? $article->voting : '-'); ?>
					
						<?php if (($article->voting) &&  $this->params->get('se_show_voting_count')) { ?>
							<?php echo " (" . $article->voting_count . " " . JText::_('NUM_VOTES') . ")"; ?>
						<?php } ?>
					</div>			
				<?php } ?>
				...
				<?php if ($this->params->get('se_show_hits') == 2) { ?>
			    	<div class="hits"><?php echo JText::_('HITS') .": ". $article->hits; ?></div>
				<?php } ?>
