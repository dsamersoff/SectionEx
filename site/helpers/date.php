...
class SectionExDateHelper
{
	function sec_to_DB_time($seconds) {
		$negative = 0;
		if ($seconds < 0){
			$seconds = abs($seconds);
			$negative = 1;
	...
	function get_user_DB_tz_offset_relative_to_joomla_tz_offset()
	{
		$user =& JFactory::getUser();
		if ($user->id != 0)
		{	// -> logged-in user
			$mainframe = JFactory::getApplication();
			$joomlaTimeZone  = $mainframe->getCfg('offset');				// joomla (global) time-zone set in the back-end
			$userTimeZone = $user->getParam('timezone', $joomlaTimeZone);	// user time-zone

			return SectionExDateHelper::sec_to_DB_time(($userTimeZone - $joomlaTimeZone) * 60 * 60);
		}

		// guests always use the global joomla time zone (no ofset)
		return SectionExDateHelper::sec_to_DB_time(0);
	}

	function getOffSet16($numberOnly = true)
	{
	...
	function getOffSet( $numberOnly	= true )
	{
	...
	public static function toFormat($jdate, $format='%Y-%m-%d %H:%M:%S')
	{
		...
		if( SectionExHelper::getJoomlaVersion() >= '1.6' )
		{
			...
			{
				$format = SectionExHelper::getHelper( 'date' )->strftimeToDate( $format );
			}
